...
if __name__ == '__main__':
    ...
    order_is_weekend_history = np.zeros(shape=[num_rows, 100], dtype=np.int8)
    order_part_of_day_history = np.zeros(shape=[num_rows, 100], dtype=np.int8)
    ...
    for i, row in product_data.iterrows():
        ...
        order_is_weekend_history[i, :], _ = pad_1d(map(int, row['order_is_weekend_history'].split()), 100)
        order_part_of_day_history[i, :], _ = pad_1d(map(int, row['order_part_of_day_history'].split()), 100)
    ...
    np.save('data/order_is_weekend_history.npy', order_is_weekend_history)
    np.save('data/order_part_of_day_history.npy', order_part_of_day_history)
